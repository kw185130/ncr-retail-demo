import {
    getSiteCatalogItemDetailsByItemCode,
    getItemDetail,
} from "~/lib/catalog";
import { getCatalogItemCategoryAncestorsByMerchandiseCategory } from "~/lib/category";

export default async function handler(req, res) {
    console.log("here");
    let siteId = req.query.params[0];
    let itemCode = req.query.params[1];
    const catalogItem = await getItemDetail(siteId, itemCode);
    console.log(catalogItem);
    if (catalogItem.status !== 200) {
        res.status(catalogItem.status).json(catalogItem);
    }
    const ancestors =
        await getCatalogItemCategoryAncestorsByMerchandiseCategory(
            catalogItem.data.pageContent[0].item.merchandiseCategory.nodeId
        );
    console.log(ancestors.status);
    if (ancestors.status !== 200) {
        res.status(ancestors.status).json(ancestors);
    }
    catalogItem.data["categories"] = ancestors.data ? ancestors.data.nodes : [];

    res.status(200).json({
        catalogItem,
        logs: [catalogItem.log, ancestors.log],
    });
}
